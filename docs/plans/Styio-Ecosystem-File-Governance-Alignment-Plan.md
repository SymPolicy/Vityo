...
`view` 当前目录结构已经具备产品化形态，但治理自动化最弱；本计划要求 `view` 对齐到与 `nightly` / `pafio` 等强的文件治理水位：
...
| `FG1` | 与 `nightly` / `pafio` 的目录职责对齐 | `docs/README.md` `docs/specs/DOCUMENTATION-POLICY.md` | docs + hygiene floor |
